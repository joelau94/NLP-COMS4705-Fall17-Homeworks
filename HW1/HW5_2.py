...
def get_unk_replace(corpus_file='ner_train.dat', min_freq=5): # get a lookup table for replacing infrequent words
	count = Counter([line.split()[0] for line in open(corpus_file,'r') if line.strip()!=''])
	unk_replace = defaultdict(lambda:'_RARE_')
	unk_replace.update({ k:k if count[k]>=min_freq else '_RARE_' for k in count.keys() })
	return unk_replace
